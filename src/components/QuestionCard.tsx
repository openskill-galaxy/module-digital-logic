    ...
    <Link to={href} className="card-hover block p-5">
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-2">
          <span className="rounded bg-brand-500/20 px-1.5 py-0.5 text-xs text-brand-100">{typeLabel(question.type)}</span>
          <DifficultyBadge difficulty={question.difficulty} />
        </div>
        <span className="text-xs text-white/40">{question.chapter}</span>
      </div>
